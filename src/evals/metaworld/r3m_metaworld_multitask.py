# =============================================================================
# r3m_metaworld_multitask.py
#
# Reproduces the MetaWorld evaluation from the R3M paper (Nair et al. 2022)
# across all 5 tasks:
#   - button-press-v3    (our working baseline from r3m_button2.py)
...
#
# CLI usage:
#   # Baseline (original pre-trained R3M):
#   python3 r3m_metaworld_multitask.py --encoder baseline
#
#   # Fine-tuned on DROID:
#   python3 r3m_metaworld_multitask.py --encoder droid \
#       --checkpoint ./checkpoints/r3m/r3m_droid_finetuned.pt
#
#   # Fine-tuned on Ego-Exo4D (when available):
#   python3 r3m_metaworld_multitask.py --encoder egoexo4d \
...
def load_encoder(encoder_type: str = "baseline", checkpoint: str = None):
    """
    Load the R3M encoder based on CLI flag.

    encoder_type:
        "baseline"  — original pre-trained R3M (no fine-tuning)
        "droid"     — R3M fine-tuned on DROID (requires --checkpoint)
        "egoexo4d"  — R3M fine-tuned on Ego-Exo4D (requires --checkpoint)
    """
    global r3m

    if encoder_type == "baseline":
        print("[Encoder] Loading baseline R3M (pre-trained, no fine-tuning)")
        r3m = load_r3m("resnet50")
    else:
        if checkpoint is None:
            raise ValueError(f"--checkpoint required for encoder type '{encoder_type}'")
        print(f"[Encoder] Loading fine-tuned R3M ({encoder_type}) from {checkpoint}")
        r3m = load_r3m("resnet50")
        state = torch.load(checkpoint, map_location=DEVICE)
        r3m.module.load_state_dict(state["encoder"])
...
VISUAL_DIM = 2048   # ResNet-50 output
...
class MetaWorldTaskRunner:
    ...
    def train(self, demo_data: list, num_steps: int = 20000,
        ...
        self.policy = R3MPolicy(proprio_dim=self.proprio_dim).to(DEVICE)
        criterion   = nn.MSELoss()
        optimizer   = torch.optim.Adam(self.policy.parameters(), lr=lr)

        print(f"\n[{self.task_key}] Training BC policy for {num_steps} steps …")
...
def save_results_csv(results: dict, demo_episodes: int, camera_ids: dict,
    ...
    if csv_path is None:
        ...
        csv_path  = f"r3m_results_{encoder_type}_demos{demo_episodes}_{timestamp}.csv"
...
def run_all_tasks(
    ...
    for task_key in tasks_to_run:
        ...
        if visualize:
            runner.visualize(
                ...
                save_path=(f"demo-size-{demo_episodes}_cam-"
    ...
    # Summary table
    print("\n" + "=" * 60)
    print(f"  RESULTS SUMMARY  [{encoder_type}]")
    ...
    # CSV filename includes encoder type for easy comparison across runs
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    cam_id    = TASK_REGISTRY[tasks_to_run[-1]]["camera_id"]
    tag_str   = f"_{tag}" if tag else ""
    csv_path  = (f"r3m_results_{encoder_type}{tag_str}_demos{demo_episodes}"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="R3M MetaWorld evaluation — baseline or fine-tuned encoder")
